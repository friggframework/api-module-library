...
class AttentiveIntegration extends IntegrationBase {
    static Definition = {
        name: 'attentive',
        version: '1.0.0',
        display: {
            label: 'AttentiveMobile',
            description: 'Attentive Mobile',
            imageURL: 'https://friggframework.org/assets/img/attentive-icon.png',
            icon: '',
            category: 'SMS',
        },
        modules: {
    ...
    async processAuthorizationCallback(params) {
        ...
        return {
            ...
            type: AttentiveIntegration.Definition.name,
        };
    }
    ...
    async getApiObject() {
        const attentiveParams = {delegate: this};

        if (this.credential) {
            attentiveParams.access_token = this.credential.access_token;
            attentiveParams.id_token = this.credential.id_token;
            attentiveParams.expires_in = this.credential.accessExpiresIn;
        }

        return new Api(attentiveParams);
    }
}

module.exports = AttentiveIntegration;